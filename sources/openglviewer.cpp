...
OpenGLViewer::OpenGLViewer(QWidget* parent)
    ...
    arcball = std::unique_ptr<ArcballController>(new ArcballController(this));
...
void OpenGLViewer::initializeGL() {
    ...
    // Initialize VAO.
    vao = std::unique_ptr<QOpenGLVertexArrayObject>(new QOpenGLVertexArrayObject(this));
    vao->create();
    vao->bind();

    vBuffer = std::unique_ptr<QOpenGLBuffer>(new QOpenGLBuffer(QOpenGLBuffer::VertexBuffer));
    vBuffer->create();
    ...
    iBuffer = std::unique_ptr<QOpenGLBuffer>(new QOpenGLBuffer(QOpenGLBuffer::IndexBuffer));
    iBuffer->create();
    ...
    texture = std::unique_ptr<QOpenGLTexture>(new QOpenGLTexture(texImage, QOpenGLTexture::MipMapGeneration::GenerateMipMaps));
    ...
    // Initialize shaders.
    shader = std::unique_ptr<QOpenGLShaderProgram>(new QOpenGLShaderProgram(this));
    ...
    dipoleShader = std::unique_ptr<QOpenGLShaderProgram>(new QOpenGLShaderProgram(this));
    ...
    gbufShader = std::unique_ptr<QOpenGLShaderProgram>(new QOpenGLShaderProgram(this));
...
void OpenGLViewer::resizeGL(int width, int height) {
    ...
    // FBOs for G-buffers.
    deferFbo = std::unique_ptr<QOpenGLFramebufferObject>(new QOpenGLFramebufferObject(this->width(), this->height(),
    ...
    dipoleFbo = std::unique_ptr<QOpenGLFramebufferObject>(new QOpenGLFramebufferObject(this->width(), this->height(),
        QOpenGLFramebufferObject::Attachment::Depth, GL_TEXTURE_2D, GL_RGBA32F));
}
...
void OpenGLViewer::calcGBuffers() {
    ...
    if (!gbufFbo) {
        gbufFbo = std::unique_ptr<QOpenGLFramebufferObject>(new QOpenGLFramebufferObject(bufSize, bufSize,
            QOpenGLFramebufferObject::Attachment::Depth, GL_TEXTURE_2D, GL_RGBA32F));
    ...
    // Prepare sample VAO.
    sampleVAO = std::unique_ptr<QOpenGLVertexArrayObject>(new QOpenGLVertexArrayObject (this));
    sampleVAO->create();
    sampleVAO->bind();

    sampleVBuf = std::unique_ptr<QOpenGLBuffer>(new QOpenGLBuffer(QOpenGLBuffer::VertexBuffer));
    sampleVBuf->create();
    ...
    sampleIBuf = std::unique_ptr<QOpenGLBuffer>(new QOpenGLBuffer(QOpenGLBuffer::IndexBuffer));
    sampleIBuf->create();
